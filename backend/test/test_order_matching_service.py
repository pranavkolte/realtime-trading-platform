...
from app.api.services.order_matching_service import (
    OrderMatchingEngine,
    TradeResult,
)
...
def test_market_order_match(engine):
    ...
    market_buy = make_order(
        Side.BUY, price=0, quantity=1.5, order_type=OrderType.MARKET
    )
...
def test_market_order_no_match(engine):
    # No sell orders in the book
    # so market buy should not match and should be cancelled or remain unfilled
    market_buy = make_order(
        Side.BUY, price=0, quantity=1.0, order_type=OrderType.MARKET
    )
    trades = engine.add_order(market_buy)
...
def test_market_order_cancelled(engine):
    ...
    # Add a market sell order with no matching buy
    # (simulate by removing buy order)
    # Remove all buy orders to simulate no liquidity
    engine._buy_orders.clear()
    market_sell = make_order(
        Side.SELL, price=0, quantity=1.0, order_type=OrderType.MARKET
    )
...
async def test_notify_trades_and_book_update(engine):
    ...
    with patch(
        "app.api.services.order_matching_service.ws_manager"
    ) as mock_ws:
...
async def test_notify_trades_exception_handling(engine):
    ...
    with patch(
        ...
        mock_ws.send_order_status_update = AsyncMock(side_effect=Exception())
...
def test_restore_from_database_with_trades(engine):
    ...
    with patch("app.database.models.trade_models.Trade"):
...
def test_limit_order_price_priority(engine):
    ...
    buy_order = make_order(
        Side.BUY, price=100.0, quantity=1.0, created_at=older_time
    )
    sell_order = make_order(
        Side.SELL, price=99.0, quantity=1.0, created_at=newer_time
    )
