...
async def test_get_price_data_nonempty(monkeypatch, mock_db):
    ...
    from app.api.routers import price_routers
    monkeypatch.setattr(price_routers.PriceHistory, "from_orm", staticmethod(lambda x: x))
    monkeypatch.setattr(price_routers, "PriceHistoryResponse", lambda price: {"price": {"id": price.id, "timestamp": price.timestamp, "value": price.value}})
