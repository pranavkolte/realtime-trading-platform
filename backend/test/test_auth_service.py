...
class TestAuthService:
        ...
        with patch.object(UserModel, '__init__', return_value=None):
            with patch.object(mock_db, 'add'), patch.object(mock_db, 'commit'):
                with patch.object(mock_db, 'refresh') as mock_refresh:
                    mock_refresh.side_effect = lambda user: setattr(user, 'user_id', example_uuid)
                    ...
                    mock_user_instance.user_type = UserTypeEnum.trader             # << valid enum
                    
                    with patch('app.api.services.auth_service.UserModel', return_value=mock_user_instance):
    ...
    async def test_login_success(self, mock_create_token, mock_verify_password):
        # Arrange
        mock_db = Mock(spec=Session)
        mock_user = Mock()
