...
async def test_verify_password_success(monkeypatch):
    monkeypatch.setattr(auth_util.password_context, "verify", lambda secret, hash: True)
    assert await auth_util.verify_password("pw", "hashed")

@pytest.mark.asyncio
async def test_verify_password_failure(monkeypatch):
    monkeypatch.setattr(auth_util.password_context, "verify", lambda secret, hash: False)
...
def test_decode_access_token_expired(monkeypatch):
    ...
    monkeypatch.setattr(auth_util.jwt, "decode", lambda *a, **k: (_ for _ in ()).throw(Expired()))
    with pytest.raises(ValueError, match="expired"):
        auth_util.decode_access_token("token")

def test_decode_access_token_invalid(monkeypatch):
    monkeypatch.setattr(auth_util.jwt, "decode", lambda *a, **k: (_ for _ in ()).throw(InvalidTokenError()))
    with pytest.raises(ValueError, match="Invalid token"):
        auth_util.decode_access_token("token")

@pytest.mark.asyncio
async def test_create_auth_token_returns_tokens(sample_user, monkeypatch):
    monkeypatch.setattr(auth_util.jwt, "encode", lambda payload, key, algorithm: f"token-{payload['sub']}")
