...
from app.api.services.order_book_service import (
    OrderBookService,
    PlaceOrderResponse,
)
...
def order_book_service(db_session):
    with patch(
        "app.api.services.order_book_service.matching_engine"
    ) as mock_engine:
...
def test_initialization_with_no_price_history_but_trades(db_session):
    ...
    db_session.query.return_value.order_by.return_value.first.side_effect = [
        None,
        DummyTrade(),
    ]
...
def test_restore_order_book_from_db(order_book_service, db_session):
    ...
    # Mock active orders
    mock_orders = [
        DummyOrder(100.0, 5.0, Side.BUY),
        DummyOrder(101.0, 3.0, Side.SELL),
    ]
    query_result = (
        db_session
        .query.return_value
    ...
    query_result.return_value = mock_orders
...
@pytest.mark.asyncio
async def test_place_market_buy_order_no_sellers(
    order_book_service, db_session
):
    """Test market buy order when no sellers available"""
    order_request = PlaceOrderRequest(
        side=Side.BUY, order_type=OrderType.MARKET, quantity=10.0
    )
    ...
    with patch(
        ...
        result = await order_book_service.place_order(
            "user-123", order_request
        )
...
@pytest.mark.asyncio
async def test_place_market_sell_order_no_buyers(
    order_book_service, db_session
):
    """Test market sell order when no buyers available"""
    order_request = PlaceOrderRequest(
        side=Side.SELL, order_type=OrderType.MARKET, quantity=10.0
    )
...
async def test_place_order_with_trades(order_book_service, db_session):
    ...
    order_request = PlaceOrderRequest(
        side=Side.BUY, order_type=OrderType.LIMIT, price=100.0, quantity=10.0
    )

    # Mock trade results
    trade_result = DummyTradeResult()

    # Create a mock trade object that
    # simulates what would be created in the database
    ...
    with patch(
        ...
        with patch(
            "app.api.services.order_book_service.Trade",
...
def test_get_last_trade_price_from_db_with_trade(
    order_book_service, db_session
):
...
def test_order_book_updates_after_place_order(order_book_service, db_session):
    ...
    order_request = PlaceOrderRequest(
        side=Side.BUY, order_type=OrderType.LIMIT, price=101.0, quantity=2.0
    )
    ...
    order_book_service.get_order_book_snapshot = MagicMock(
        return_value={
