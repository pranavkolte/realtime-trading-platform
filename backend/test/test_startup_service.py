...
def mock_matching_engine():
    with patch(
        "app.api.services.startup_service.matching_engine"
    ) as mock_engine:
        yield mock_engine
...
def mock_get_db_session(mock_db_session):
    with patch(
        "app.api.services.startup_service.get_db_session",
        return_value=iter([mock_db_session]),
    ):
        yield


def test_restore_matching_engine_success(
    mock_get_db_session, mock_db_session, mock_matching_engine
):
    # Mock last trade
    ...
    mock_matching_engine.restore_from_database.assert_called_once_with(
        active_orders, mock_db_session
    )
    mock_db_session.close.assert_called_once()


def test_restore_matching_engine_no_last_trade(
    mock_get_db_session, mock_db_session, mock_matching_engine
):
    # No last trade
...
def test_restore_matching_engine_db_exception(
    mock_get_db_session, mock_db_session, mock_matching_engine
):
