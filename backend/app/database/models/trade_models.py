...
class Trade(Base):
    ...
    buy_order_id = Column(
        PG_UUID(as_uuid=True), ForeignKey("orders.order_id"), nullable=False
    )
    ...
    buyer = relationship(
        "UserModel", foreign_keys=[buy_user_id], back_populates="trades_bought"
    )
    seller = relationship(
        "UserModel", foreign_keys=[sell_user_id], back_populates="trades_sold"
    )
